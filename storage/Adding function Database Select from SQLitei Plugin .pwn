// include plugin sqlitei from https://github.com/oscar-broman/sqlitei
...
public OnGameModeInit()
{
	...
	// load trees
	new iddzz, Float: possz[3], Float: rottz[3];
	stmt_bind_result_field(LoadTrees, 0, DB::TYPE_INTEGER, iddzz);
	stmt_bind_result_field(LoadTrees, 1, DB::TYPE_FLOAT, possz[0]);
	stmt_bind_result_field(LoadTrees, 2, DB::TYPE_FLOAT, possz[1]);
	stmt_bind_result_field(LoadTrees, 3, DB::TYPE_FLOAT, possz[2]);
	stmt_bind_result_field(LoadTrees, 4, DB::TYPE_FLOAT, rottz[0]);
	stmt_bind_result_field(LoadTrees, 5, DB::TYPE_FLOAT, rottz[1]);
	stmt_bind_result_field(LoadTrees, 6, DB::TYPE_FLOAT, rottz[2]);
	...
	if(stmt_execute(LoadTrees))
	{
	    ...
	    {
            TreeData[iddzz][treeX] = possz[0];
		 	TreeData[iddzz][treeY] = possz[1];
		  	TreeData[iddzz][treeZ] = possz[2];
	        TreeData[iddzz][treeRX] = rottz[0];
		 	TreeData[iddzz][treeRY] = rottz[1];
		  	TreeData[iddzz][treeRZ] = rottz[2];

		    TreeData[iddzz][treeObjID] = CreateDynamicObject(657, TreeData[iddzz][treeX], TreeData[iddzz][treeY], TreeData[iddzz][treeZ], TreeData[iddzz][treeRX], TreeData[iddzz][treeRY], TreeData[iddzz][treeRZ]);
            CreateDynamicCP(TreeData[iddzz][treeX], TreeData[iddzz][treeY], TreeData[iddzz][treeZ], 1.5, 0, 0, -1, 5.0);
			format(label, sizeof(label), "Pohon\n(ID : %d)\n\n{FFFFFF}Gunakan cmd {F1C40F}/cut {FFFFFF}untuk menebang pohon", iddzz);
			TreeData[iddzz][treeLabel] = CreateDynamic3DTextLabel(label, 0x2ECC71FF, TreeData[iddzz][treeX], TreeData[iddzz][treeY], TreeData[iddzz][treeZ] + 1.5, 5.0);

			Iter_Add(Trees, iddzz);
		}

		printf("  [Lumberjack by richo and rootcause] Loaded %d trees.", Iter_Count(Trees));
	}

	// load buyers
	new skinnzz, Float: bpos[4];
	stmt_bind_result_field(LoadBuyers, 0, DB::TYPE_INTEGER, iddzz);
	stmt_bind_result_field(LoadBuyers, 1, DB::TYPE_INTEGER, skinnzz);
	...
	if(stmt_execute(LoadBuyers))
	{
	    ...
	    {
            BuyerData[iddzz][buyerSkin] = skinnzz;
		 	BuyerData[iddzz][buyerX] = bpos[0];
		  	BuyerData[iddzz][buyerY] = bpos[1];
	        BuyerData[iddzz][buyerZ] = bpos[2];
		 	BuyerData[iddzz][buyerA] = bpos[3];

		    BuyerData[iddzz][buyerActorID] = CreateActor(BuyerData[iddzz][buyerSkin], BuyerData[iddzz][buyerX], BuyerData[iddzz][buyerY], BuyerData[iddzz][buyerZ], BuyerData[iddzz][buyerA]);
            SetActorInvulnerable(BuyerData[iddzz][buyerActorID], 1);

			format(label, sizeof(label), "Lumberjack Bos\n(ID : %d)\n\n{FFFFFF}Gunakan cmd {F1C40F}/chainsaw {FFFFFF}untuk membeli chainsaw dengan harga {2ECC71}$%d.\n{FFFFFF}Gunakan cmd {F1C40F}/log sell {FFFFFF}untuk menjual kayu kamu per 1 kayu seharga {2ECC71}$%d", iddzz, CSAW_PRICE, LOG_PRICE);
			BuyerData[iddzz][buyerLabel] = CreateDynamic3DTextLabel(label, 0xF1C40FFF, BuyerData[iddzz][buyerX], BuyerData[iddzz][buyerY], BuyerData[iddzz][buyerZ] + 0.25, 5.0, .testlos = 1);

			Iter_Add(Buyers, iddzz);
		}
	...
	new idd, model, password[GATE_PASS_LEN], Float: poss[3], Float: rott[3], Float: openpos[3], Float: openrot[3];
	stmt_bind_result_field(LoadGates, 0, DB::TYPE_INTEGER, idd);
	...
	stmt_bind_result_field(LoadGates, 3, DB::TYPE_FLOAT, poss[0]);
	stmt_bind_result_field(LoadGates, 4, DB::TYPE_FLOAT, poss[1]);
	stmt_bind_result_field(LoadGates, 5, DB::TYPE_FLOAT, poss[2]);
	stmt_bind_result_field(LoadGates, 6, DB::TYPE_FLOAT, rott[0]);
	stmt_bind_result_field(LoadGates, 7, DB::TYPE_FLOAT, rott[1]);
	stmt_bind_result_field(LoadGates, 8, DB::TYPE_FLOAT, rott[2]);
	...
	if(stmt_execute(LoadGates))
	{
	    ...
	    {
            GateData[idd][GateModel] = model;
			GateData[idd][GatePassword] = password;
			GateData[idd][GatePos][0] = poss[0];
			GateData[idd][GatePos][1] = poss[1];
			GateData[idd][GatePos][2] = poss[2];
			GateData[idd][GateRot][0] = rott[0];
			GateData[idd][GateRot][1] = rott[1];
			GateData[idd][GateRot][2] = rott[2];
			GateData[idd][GateOpenPos][0] = openpos[0];
			GateData[idd][GateOpenPos][1] = openpos[1];
			GateData[idd][GateOpenPos][2] = openpos[2];
			GateData[idd][GateOpenRot][0] = openrot[0];
			GateData[idd][GateOpenRot][1] = openrot[1];
			GateData[idd][GateOpenRot][2] = openrot[2];

			GateData[idd][GateObject] = CreateDynamicObject(model, poss[0], poss[1], poss[2], rott[0], rott[1], rott[2]);
			Iter_Add(Gates, idd);
		}
	}
	...
	// Load Robberies
	new id, name[32], Float: pos[3], amount, reqt, safet, recamount, recby[MAX_PLAYER_NAME];
	...
	stmt_bind_result_field(LoadRobberies, 5, DB::TYPE_INTEGER, -1);
	stmt_bind_result_field(LoadRobberies, 6, DB::TYPE_INTEGER, -1);
	...
	if(stmt_execute(LoadRobberies))
	{
	    ...
	    while(stmt_fetch_row(LoadRobberies))
	    {
			...
			RobberyData[id][IntID] = -1;
			RobberyData[id][VWID] = -1;
			...
			RobberyData[id][Checkpoint] = CreateDynamicCP(pos[0], pos[1], pos[2], 1.25, -1, -1, .streamdistance = 5.0);

		    format(label, sizeof(label), "{B0C4DE}[ID: %d]\n\n{FFFFFF}%s\n{2ECC71}%s - %s\n{FFFFFF}Bisa untuk dirob", id, name, formatInt(floatround(amount / 4)), formatInt(amount));
		    RobberyData[id][Label] = CreateDynamic3DTextLabel(label, 0xF1C40FFF, pos[0], pos[1], pos[2] + 0.25, 10.0, _, _, 1, -1, -1);
    ...
    {
	    FamInfoByricho[idx][FamilyTakenByricho] = 0;
		format(FamInfoByricho[idx][FamilyNameByricho], 32, "None");
		format(FamInfoByricho[idx][FamilyMotdByricho], 64, "");
		FamInfoByricho[idx][FamilyColorByricho] = 0;
		format(FamInfoByricho[idx][FamilyPemimpinByricho], MAX_PLAYER_NAME, "None");
		FamInfoByricho[idx][FamilyMembersByricho] = 0;
		format(FamInfoByricho[idx][fRank1], 32, "");
		format(FamInfoByricho[idx][fRank2], 32, "");
		format(FamInfoByricho[idx][fRank3], 32, "");
		format(FamInfoByricho[idx][fRank4], 32, "");
		format(FamInfoByricho[idx][fRank5], 32, "");
		format(FamInfoByricho[idx][fRank6], 32, "");
	}
	LoadShelterStorage();
    LoadBoards();
    LoadFamilies();
...
// data inlude from https://github.com/oscar-broman/sqlitei/blob/master/sqlitei.inc
