...
public OnGameModeInit()
{
  ...
  SetDynamicObjectMaterialText(AuctionMartChartBoard, 0,"{FFFFFF}Auction Mart",  130, "Arial", 70, 1, -1250582, 0, 1);
	...
	*/
	...
	ConnectMySQL();
	SetGameModeText("Beta Test Putu");
	...
	*/
    ...
    if(dialogid == DIALOG_GANTINAMA)
    {
        ...
        if(response) //If the player clicked login
        {
        	...
        	mysql_query(MySQL:g_Sql, passworddariuser);
        	cache_get_value_int(0, "id", pInfo[playerid][ID]);
...
CMD:go(playerid, params[])
{
  ...
  ID, Float:x, Float:y, Float:z;
    ...
    if(sscanf(params, "u", ID)) return SendClientMessage(playerid, COLOR_ERROR, "[Usage:] /goto [partofname/playerid].");
